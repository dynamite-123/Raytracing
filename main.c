...
void DrawCloseButton(SDL_Surface* surface, struct Button button, Uint32 color) {
    ...
    for (int i = 0; i < button.width / 8; i++) {
        SDL_Rect line2 = {
            button.x + button.width - padding - i, 
            button.y + padding, 
            1, 
            button.height - 2 * padding
        };
...
void FillRays(SDL_Surface *surface, struct Ray rays[RAYS_NUMBER], Uint32 color, struct Circle object) {
    ...
    for (int i=0; i<RAYS_NUMBER; i++){
        ...
        while (end_of_screen != 1 && object_hit != 1) {
            ...
            SDL_Rect ray_point = (SDL_Rect) {x_draw, y_draw, RAYS_THICKNESS, RAYS_THICKNESS};
    ...
    while (simulation_running) {
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN) {
                ...
                if (IsPointInButton(event.button.x, event.button.y, closeButton)) {
                    simulation_running = 0;
                }
            }
        ...
        if(shadow_circle.x - shadow_circle.radius < 0 || shadow_circle.x + shadow_circle.radius > WIDTH)
